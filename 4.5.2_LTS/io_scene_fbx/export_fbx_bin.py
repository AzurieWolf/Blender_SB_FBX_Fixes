...
def fbx_data_light_elements(root, lamp, scene_data):
    ...
    do_light = True
    do_shadow = getattr(lamp, "use_shadow", False)
    shadow_color = (0.0, 0.0, 0.0)
    if lamp.type not in {'HEMI'}:
        do_light = True
        do_shadow = getattr(lamp, "use_shadow", False)
        shadow_color = (0.0, 0.0, 0.0)  # Default or placeholder color
    ...
    if do_shadow:
        elem_props_template_set(tmpl, props, "p_bool", b"CastShadows", True)
        elem_props_template_set(tmpl, props, "p_color", b"ShadowColor", shadow_color)
    else:
        elem_props_template_set(tmpl, props, "p_bool", b"CastShadows", False)
